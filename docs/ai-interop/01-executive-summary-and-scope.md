...
本项目面向基于 Code - OSS / VS Code 内核构建自研 IDE 的平台作者场景。目标不是让扩展 A 直接调用扩展 B 的公开 API，而是把多个 AI 扩展、AI Agent、CLI Agent、Chat UI、Webview UI 之间的双向消息通信、会话管理、流式响应、工具调用协调提升为平台级能力。平台级实现的原因在于：

- VS Code 扩展运行在独立 Extension Host 中；
- 平台天然存在 local / web / remote 多宿主；
- 扩展无法直接访问主界面 DOM，也不应通过 UI hack 进行互通；
- 扩展间互通若不经平台仲裁，将无法获得一致的权限、路由、审计与恢复语义。:contentReference[oaicite:3]{index=3}:contentReference[oaicite:4]{index=4}
...
6. 支持最小闭环：
   ...
   - 平台支持取消；
   - 平台记录结构化事件；
   - 平台能够拒绝未授权调用。:contentReference[oaicite:5]{index=5}:contentReference[oaicite:6]{index=6}
...
5. 不做所有现有生态的全量自动兼容；
6. 不做完整企业治理能力；
7. 不做最终版审计中心和运营后台。:contentReference[oaicite:7]{index=7}:contentReference[oaicite:8]{index=8}
...
- 对 remoteAuthority 不一致场景，本期按“显式拒绝”处理。:contentReference[oaicite:9]{index=9}

### 5.2 API 边界
- 首阶段只提供 Proposed API；
- 仅暴露最小原语，不暴露复杂编排 DSL；
- API 以 endpoint / session / invocation 为中心，不直接绑定某个具体 UI。:contentReference[oaicite:10]{index=10}:contentReference[oaicite:11]{index=11}
...
5. 平台可拒绝未授权跨扩展调用；
6. 平台可为一次调用记录结构化审计事件。:contentReference[oaicite:12]{index=12}

### 6.2 工程成功标准
1. 代码落点符合 Workbench / API / Contrib 分层；
2. 不通过 hack 修改现有 chat/webview/DOM 机制；
3. 与现有 MainThread / ExtHost / rpcProtocol 模式一致；
4. 可在 Extension Development Host 中自动化测试。:contentReference[oaicite:13]{index=13}
